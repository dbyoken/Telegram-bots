...
def regions_kb() -> InlineKeyboardMarkup:
    kb = []
    row = []
    for i, v in enumerate(VILOYATLAR, start=1):
        row.append(InlineKeyboardButton(text=v, callback_data=f"region:{v}"))
        if i % 2 == 0:
            kb.append(row)
            row = []
    if row:
...
def branches_kb() -> InlineKeyboardMarkup:
    kb = [[InlineKeyboardButton(text=info["nomi"], callback_data=f"book_branch:{key}")] for key, info in FILIALLAR.items()]
    return InlineKeyboardMarkup(inline_keyboard=kb)
...
async def cart_checkout(callback: CallbackQuery, state: FSMContext):
    ...
    if not carts.get(user_id):
        ...
        return
    await callback.message.answer("Qaysi viloyatdansiz?", reply_markup=regions_kb())
    await state.set_state(Order.choosing_region)
    await callback.answer()


@router.callback_query(Order.choosing_region, F.data.startswith("region:"))
async def order_region_chosen(callback: CallbackQuery, state: FSMContext):
...
async def booking_start(message: Message, state: FSMContext):
    await message.answer("Qaysi filialdan joy band qilmoqchisiz?", reply_markup=branches_kb())
