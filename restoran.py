...
ADMIN_CHAT_ID = os.getenv("ADMIN_CHAT_ID")  # Buyurtmalar shu Telegram ID'ga yuboriladi
...
# Joy band qilish uchun soatlar (08:00 dan 01:00 gacha)
TIME_SLOTS = [f"{h:02d}:00" for h in list(range(8, 24)) + [0, 1]]
...
# ---------------------------------------------------------------------------
# Persistent storage helpers (JSON)
# ---------------------------------------------------------------------------
...
def load_reservations() -> dict:
    if RESERVATIONS_FILE.exists():
        with open(RESERVATIONS_FILE, "r", encoding="utf-8") as f:
            return json.load(f)
    return {"reservation_counter": 0, "active_reservations": {}}


def save_reservations(data: dict) -> None:
...
def reservation_time_kb() -> ReplyKeyboardMarkup:
    rows = []
...
# ---------------------------------------------------------------------------
# Aksiyalarni ko'rsatish (bosh menyudan) — endi tugmalar bilan
# ---------------------------------------------------------------------------
@router.message(StateFilter(None), F.text == "🔥 Aksiyalar")
async def aksiya_info(message: Message, state: FSMContext):
    lines = []
    for item in AKSIYA:
        lines.append(
            f"🔥 <b>{item['name']}</b>\n"
            f"   <s>{item['old_price']:,} so'm</s> → <b>{item['price']:,} so'm</b>".replace(",", " ")
        )
    await state.update_data(cart=[])
    await state.set_state(OrderStates.choosing_item)
    await state.update_data(current_category="__aksiya__", quick_order=True)
    await message.answer(
        "Joriy aksiyalar:\n\n" + "\n\n".join(lines) + "\n\nBuyurtma qilish uchun taomni tanlang 👇",
...
async def start_order(message: Message, state: FSMContext):
    ...
    await state.update_data(cart=[], quick_order=False)
...
async def branch_chosen(message: Message, state: FSMContext):
    data = await state.update_data(branch=message.text)

    # Agar aksiya orqali tez buyurtma boshlangan bo'lsa, to'g'ridan-to'g'ri miqdor so'raladi
    if data.get("pending_item"):
        item = data["pending_item"]
        await state.set_state(OrderStates.entering_quantity)
        await message.answer(
            f"✅ Viloyat: {message.text}\n\n{item['name']} — nechta dona?",
...
async def item_chosen(message: Message, state: FSMContext):
    ...
    if data.get("quick_order") and not data.get("branch"):
        # Aksiya orqali tez buyurtma: avval viloyat so'raladi
        await state.set_state(OrderStates.choosing_branch)
        await message.answer(
            f"✅ Tanlandi: {item['name']}\n\nQaysi viloyatga yetkazib berish kerak?",
...
async def location_received(message: Message, state: FSMContext):
    ...
    if ADMIN_CHAT_ID:
        ...
        admin_text = (
            ...
            f"👤 Mijoz: {user.full_name} ({username_part})\n"
        ...
        try:
            ...
            logger.exception(f"Adminga #{order_id} buyurtmasini yuborishda xatolik")
    else:
        logger.warning("ADMIN_CHAT_ID o'rnatilmagan — buyurtma adminga yuborilmadi")
...
# ---------------------------------------------------------------------------
# Joy band qilish jarayoni
# ---------------------------------------------------------------------------
@router.message(F.text == "📅 Joy band qilish")
async def start_reservation(message: Message, state: FSMContext):
    await state.set_state(ReservationStates.choosing_branch)
    await message.answer("Qaysi filialdan joy band qilmoqchisiz?", reply_markup=branch_kb())


@router.message(ReservationStates.choosing_branch, F.text.in_(BRANCHES))
async def reservation_branch_chosen(message: Message, state: FSMContext):
    await state.update_data(res_branch=message.text)
...
@router.message(ReservationStates.choosing_branch, F.text == "⬅️ Orqaga")
async def reservation_branch_back(message: Message, state: FSMContext):
    await state.clear()
    await message.answer("Bosh menyu:", reply_markup=main_menu_kb())


@router.message(ReservationStates.choosing_time, F.text.in_(TIME_SLOTS))
async def reservation_time_chosen(message: Message, state: FSMContext):
    await state.update_data(res_time=message.text)
    await state.set_state(ReservationStates.entering_name)
    await message.answer("Ism-familyangizni kiriting:", reply_markup=ReplyKeyboardRemove())


@router.message(ReservationStates.choosing_time, F.text == "⬅️ Orqaga")
async def reservation_time_back(message: Message, state: FSMContext):
    await state.set_state(ReservationStates.choosing_branch)
    await message.answer("Qaysi filialdan joy band qilmoqchisiz?", reply_markup=branch_kb())


@router.message(ReservationStates.entering_name)
async def reservation_name_entered(message: Message, state: FSMContext):
...
async def reservation_phone_received(message: Message, state: FSMContext):
    data = await state.update_data(res_phone=message.contact.phone_number)

    reservations = load_reservations()
    reservations["reservation_counter"] += 1
    res_id = reservations["reservation_counter"]
    reservations["active_reservations"][str(res_id)] = {
        "user_id": message.from_user.id,
        "branch": data.get("res_branch"),
        "time": data.get("res_time"),
        "name": data.get("res_name"),
        "phone": data.get("res_phone"),
        "status": "yangi",
    }
    ...
    if ADMIN_CHAT_ID:
        ...
        username_part = f"@{user.username}" if user.username else "username yo'q"
        admin_text = (
            f"📅 <b>Yangi joy band qilish #{res_id}</b>\n\n"
            f"👤 Mijoz: {data.get('res_name')} ({username_part})\n"
            f"📞 Tel: {data.get('res_phone')}\n"
            f"🏠 Filial: {data.get('res_branch')}\n"
            f"🕒 Soat: {data.get('res_time')}"
        )
        try:
            await message.bot.send_message(chat_id=ADMIN_CHAT_ID, text=admin_text)
        except Exception:
            logger.exception(f"Adminga #{res_id} joy band qilishni yuborishda xatolik")
    else:
        logger.warning("ADMIN_CHAT_ID o'rnatilmagan — joy band qilish adminga yuborilmadi")

    await state.clear()
    await message.answer(
        f"✅ Joyingiz band qilindi!\n"
        f"Band qilish raqami: #{res_id}\n"
        f"🏠 Filial: {data.get('res_branch')}\n"
        f"🕒 Soat: {data.get('res_time')}\n\n"
        "Tez orada operator siz bilan bog'lanadi.",
        reply_markup=main_menu_kb(),
